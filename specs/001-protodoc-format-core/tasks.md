...
| T-0393 | DEFECT-2026-09-19c fix: merge wires the CON-024 (retention-point crossing) and FR-096 (erased-unit replay) guards, closing the last disclosed merge gap — a new HISTORY-segment ERASURE_RECORD reader plus real commit-ring RetentionPoint decode feed the already-existing pkg/merge.CheckRetentionPoint/ErasureIndex primitives, refusing each condition by name against real per-branch CONTENT-segment ordinals and content digests | TR-012, CON-024, FR-096 | T-0391 | hephaestus | `TestCON_024_MergeRefusesAcrossRetentionPoint`, `TestFR_096_MergeRefusesReplayOfErasedUnit` (integration) |
| T-0394 | DEFECT-2026-09-20: verify never stripped an ATTEST segment's leading ledger segment header before decoding, so `integrity.DecodeSignatureRecord` failed on EVERY real ATTEST segment (including a genuine SIGNATURE), reporting "unverified" for all of them and one signatures-array entry per ATTEST segment instead of per cli.md S5's one-per-SIGNATURE-frame contract; fixed by reusing attestreader.go's own framing helpers and dispatching on the real discriminant, skipping ATTESTATION_EVIDENCE records entirely | TR-012, cli.md S5 | T-0374 | hephaestus | `TestTR_012_VerifyDistinguishesSignatureFromEvidence` (integration) |
| T-0395 | DEFECT-2026-09-20b, found via an end-to-end CLI battery against real fixtures: diff classified changes by comparing `SegmentTableSlot.Digest`, a field no writer path ever populates for CONTENT segments (left zero), so two documents with the same segment count but genuinely different content always compared equal and were reported "identical"; also contradicted diff.go's own contract (construct-level, not storage-unit-level) by keying on storage ordinal. Fixed by decoding real CONTENT via `extract.LoadContentRecords` keyed by authored unit-id (the same identity merge/project/redact/publish/verify already use), comparing real frame bytes | TR-012, TR-002 | T-0389 | hephaestus | `TestTR_012_DiffVerbReadsRealFiles` (integration, fixture rebuilt with real decodable content) |
| T-0396 | FINDING-2026-09-20 fix: wire `validate`'s real, already-implemented, already-tested `pkg/validate.CheckStorageIntegrityTree` (FR-104/FR-105) into `realValidateStepsFor` so `validate` actually recomputes each live segment's real digest and T_S against the winning ring record's `ledger_root`, naming the offending ordinal on a mismatch; requires first auditing/fixing every test fixture across the suite that currently leaves `SegmentTableSlot.Digest` unpopulated (zero) for a CONTENT/HISTORY/ATTEST segment, since a correct wiring will flip any fixture with a wrong-or-zero digest from passing `validate` to failing it — this is the majority of the work, not the check-wiring itself | TR-012, FR-104, FR-105 | T-0373 | hephaestus | `TestTR_012_ValidateDetectsRealDigestMismatch` (integration); full suite must stay green with correct (non-zero, matching) digests in every fixture, not by skipping the new check |
